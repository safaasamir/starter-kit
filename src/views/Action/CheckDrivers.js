...
import CheckDriver from '../tables/CheckDriver';
function CheckDrivers() {
    ...
    const loadUserData = async (start, end, increas, optType = null) => {
        switch (optType) {
            case "search":
                ...
                return await axios
                    .get(`http://localhost:5000/Check?q=${value}&_start=${start}&_end=${end}`)
                    .then((response) => {
            ...
            default:
                return await axios
                    .get(`http://localhost:5000/Check?_start=${start}&_end=${end}`)
    ...
    return (
        ...
        <CheckDriver onsubmit={handleSearch} onclick={handleReset} setvalue={setValue} values={value} dataTable={data} prev={renderPagination()}/>
