...
function Bus() {
    ...
    const loadUserData = async (start, end, increas, optType = null) => {
        switch (optType) {
            case "search":
                ...
                return await axios
                    .get(`http://localhost:5000/bus?q=${value}&_start=${start}&_end=${end}`)
                    .then((response) => {
            ...
            default:
                return await axios
                    .get(`http://localhost:5000/bus?_start=${start}&_end=${end}`)
    ...
    return (
        ...
        <BusTable onsubmit={handleSearch} onclick={handleReset} setvalue={setValue} values={value} dataTable={data} prev={renderPagination()}/>
